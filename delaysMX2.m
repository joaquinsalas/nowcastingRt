...
classdef delaysMX2
    ...
    methods
        ...
        function num = plotDelays(obj)
            ...
            %set(gca, 'FontSize', 14)
            xlabel('días',  'Interpreter','LaTex','FontSize', 14)
            ylabel('positivos',  'Interpreter','LaTex','FontSize', 14)
            axis([1,m,0, max(num(:))])
        ...
        function    process(obj)
            ...
            mesh(num)
            set(gca, 'FontSize', 16)
            ylabel('reportado',  'Interpreter','LaTex','FontSize', 16)
            xlabel('actualiza',  'Interpreter','LaTex','FontSize', 16)
            ...
            m = 1;
            %despliega los casos de covid del presente al pasado
            for i=rows- k + 1:-1:70
                disp([i,k])
                %disp(t)
                if cols - k > 0
                    y = ngx(i, cols-k:cols);%why one row before rows - k + 1
                    v = num(i, cols-k:cols);
                    c = rand(1,3);
                    figure(1)
                    hold on
